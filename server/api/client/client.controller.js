...
function index(req, res) {
  return _sqldb.Client.findAll().then(respondWithResult(res)).catch(handleError(res));
}

// Gets a single Client from the DB
function show(req, res) {
  return _sqldb.Client.find({
    where: {
      _id: req.params.id
    }
...
function create(req, res) {
  return _sqldb.Client.create(req.body).then(respondWithResult(res, 201)).catch(handleError(res));
}
