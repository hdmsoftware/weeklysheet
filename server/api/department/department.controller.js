...
function index(req, res) {
  return _sqldb.Department.findAll().then(respondWithResult(res)).catch(handleError(res));
}

// Gets a single Department from the DB
function show(req, res) {
  return _sqldb.Department.find({
    where: {
      _id: req.params.id
    }
...
function create(req, res) {
  return _sqldb.Department.create(req.body).then(respondWithResult(res, 201)).catch(handleError(res));
}
